...
namespace Sitecore.SharedSource.Microsoft.Ajax.Utilities.Css
{
    ...
    public class ColorSlice
    {
        ...
        private readonly ColorName[] _colorArray;
        ...
        [SuppressMessage("Microsoft.Performance", "CA1809:AvoidExcessiveLocals",
            Justification = "there are a lot of colors")]
        private ColorSlice()
        {
            _colorArray = new[]
            {
        ...
        {
            public static readonly Dictionary<string, string> Data = Create(NestedFactory.Instance);
        ...
        private static class NestedStrictHexShorterThanNameAndAllNonStrict
        {
            ...
            private static Dictionary<string, string> Create(ColorSlice singleton)
            {
                return (from colorName in singleton._colorArray
                    where
                        (colorName.Strict && colorName.Name.Length > colorName.Hex.Length) || colorName.Strict == false
                    select colorName).DistinctBy(c => c.Name).ToDictionary(p => p.Name, p => p.Hex);
            }
        }
